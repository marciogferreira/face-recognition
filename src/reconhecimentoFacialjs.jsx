...
const ReconhecimentoFacial = props => {
    ...
    const getAngleInDegrees = angle => (angle * 180) / Math.PI

    const loadStream = async () => {
        const cameraOptions = { audio: false, video: { facingMode: "user" } }
    ...
    const onFaceFound = (curranteFace, img) => {
        log(
            `detected face: ${curranteFace.gender} ${curranteFace.age ||
            0}y distance ${100 * (curranteFace.distance || 0)}cm/`
        )
    ...
    const drawElipse = force => {
        ...
        ctx?.ellipse(
            videoSize.width / 2,
            videoSize.height / 2,
            options.boxSizeWidth,
            options.boxSizeHeight,
            0,
            0,
            2 * Math.PI
        )
        ...
        ctx?.strokeText(
            text,
            videoSize.width / 2 - text.length * 5,
            videoSize.height - 10
        )
        ctx?.fillText(
            text,
    ...
    const checkFace = async () => {
        if (isCapturing) {
            ...
            if (faceValidator.faceCount.status) {
                ...
                if (faceValidator.blinkDetected.status && blink.time === 0)
                ...
                faceValidator.faceConfidence.val =
                    human.result.face[0].faceScore || human.result.face[0].boxScore || 0
                faceValidator.faceConfidence.status =
                    faceValidator.faceConfidence.val >= options.minConfidence
                faceValidator.antispoofCheck.val = human.result.face[0].real || 0
                faceValidator.antispoofCheck.status =
                    faceValidator.antispoofCheck.val >= options.minConfidence
                faceValidator.livenessCheck.val = human.result.face[0].live || 0
                faceValidator.livenessCheck.status =
                    faceValidator.livenessCheck.val >= options.minConfidence
                faceValidator.faceSize.val = Math.min(
                    human.result.face[0].box[2],
                    human.result.face[0].box[3]
                )
                faceValidator.faceSize.status =
                    faceValidator.faceSize.val >= options.minSize
                faceValidator.distance.val = human.result.face[0].distance || 0
                faceValidator.distance.status =
                    faceValidator.distance.val >= options.distanceMin &&
                    faceValidator.distance.val <= options.distanceMax
                faceValidator.descriptor.val =
                    human.result.face[0].embedding?.length || 0
                ...
                faceValidator.gender.status =
                    faceValidator.gender.val >= options.minConfidence
                ...
                faceValidator.boxWidth.status =
                    faceValidator.boxWidth.val <= options.boxSizeWidth &&
                    faceValidator.boxWidth.val >= options.boxSizeWidth * 0.7
                faceValidator.boxHeight.status =
                    faceValidator.boxHeight.val <= options.boxSizeHeight &&
                    faceValidator.boxHeight.val >= options.boxSizeHeight * 0.7

                faceValidator.faceAngleInRange.val = [
                    Math.abs(getAngleInDegrees(human.result.face[0].rotation.angle?.yaw)),
                    Math.abs(
                        getAngleInDegrees(human.result.face[0].rotation.angle?.pitch)
                    )
                ]
                log(
                    "Face Angle:( yaw: " +
                    faceValidator.faceAngleInRange.val[0] +
                    " - pitch: " +
                    faceValidator.faceAngleInRange.val[1]
                )
                log("Face maxAngle: " + options.maxFaceAngleRange)

                faceValidator.faceAngleInRange.status =
                    faceValidator.faceAngleInRange.val[0] <= options.maxFaceAngleRange &&
                    faceValidator.faceAngleInRange.val[1] <= options.maxFaceAngleRange
            }
